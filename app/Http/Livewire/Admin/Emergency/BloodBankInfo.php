...
class BloodBankInfo extends Component
{
    ...
    {
        $info = bloodbank_table::find($this->ambulance_id);
    ...
    {
        bloodbank_table::find($this->delete_id)->delete();
        $this->emit('storeSomething');
    }
    ...
    {
        $allBloodBank = bloodbank_table::orderBy($this->sortBy,$this->sortDirection)->search(trim($this->search))->paginate($this->paginate);
        ...
        return view('livewire.admin.emergency.blood-bank-info',compact('allBloodBank','data'))->layout('layouts.admin_base');
    }
}
